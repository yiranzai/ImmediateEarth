...
fn update_earth_image(app: tauri::AppHandle) -> Result<String, String> {
    ...
    let img = image::open(path).map_err(|e| format!("读取拼接后大图失败: {}", e))?;
    ...
    println!(
        "【地球大图】加黑边后图片已保存: {}",
        black_img_path.to_string_lossy()
    );
...
fn get_weather(city: String, key: String) -> Result<String, String> {
    ...
    let weather: WeatherResp =
        serde_json::from_str(&resp).map_err(|e| format!("解析OpenWeather响应失败: {}", e))?;
...
fn get_all_monitors(app: AppHandle) -> Result<String, String> {
    ...
    let monitor_infos: Vec<MonitorInfo> = monitors
        ...
        .collect();
    serde_json::to_string(&monitor_infos).map_err(|e| format!("序列化显示器信息失败: {}", e))
}
...
async fn set_wallpaper_for_all_monitors(
    ...
    for &index in &target_indexes {
        if let Some(monitor) = monitors.get(index) {
            ...
            wallpaper::set_wallpaper_for_monitor(cropped_path.clone(), platform.clone(), index)
                .await?;
    ...
    Ok(serde_json::to_string(&results).map_err(|e| format!("序列化结果失败: {}", e))?)
}
...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            app_handle.on_menu_event(move |app, event| match event.id().0.as_str() {
                "show" => {
                    if let Some(window) = app.get_webview_window("main") {
                        let _ = window.show();
                        let _ = window.set_focus();
            ...
            let main_menu = SubmenuBuilder::with_id(app, "main_menu", "主菜单")
                .items(&[&cust_menu])
                .build()?;
            let menu = MenuBuilder::new(app).items(&[&main_menu]).build()?;
            app.set_menu(menu)?;
            app.on_menu_event(move |app, event| {
                if event.id() == cust_menu.id() {
                    let _ = app
                        .opener()
                        .open_url("https://github.com/tauri-apps/tauri", None::<&str>);
                }
            });
